...
double PtSegDistance(double query_x, double query_y, double start_x, double start_y, double end_x,
                     double end_y, double length) {
...
Box2d::Box2d(const Vec2d &point, double heading, double front_length, double back_length,
             double width)
    ...
    center_ =
        Vec2d(point.x() + cos_heading_ * delta_length, point.y() + sin_heading_ * delta_length);
    InitCorners();
}
...
std::string Box2d::DebugString() const {
    ...
    return "box2d ( center = " + center_.DebugString() + " heading = " + std::to_string(heading_) +
           " length = " + std::to_string(length_) + " width = " + std::to_string(width_);
}
