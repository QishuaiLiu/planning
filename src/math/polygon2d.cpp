...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d> &points, Polygon2d *const polygon) {
    ...
    for (int i = 0; i < n + n; ++i) {
        ...
        while (count > last_count && CrossProd(points[results[count - 2]],
                                               points[results[count - 1]], pt) <= kMathEpsilon) {
            results.pop_back();
            --count;
        }
...
bool Polygon2d::GetOverlap(const LineSegment2d &line_segment, Vec2d *const first,
...
std::string Polygon2d::DebugString() const {
    ...
    return "polygon2d ( num_points = " + std::to_string(num_points_) + " points = (";
}
