...
Node3d::Node3d(double x, double y, double phi, const std::vector<double> &XYbounds,
    ...
    x_grid_ =
        static_cast<int>((x_ - XYbounds[0]) / open_space_conf.warm_start_config.xy_grid_resolution);
    y_grid_ =
        static_cast<int>((y_ - XYbounds[2]) / open_space_conf.warm_start_config.xy_grid_resolution);
    phi_grid_ =
        static_cast<int>((phi_ - (-M_PI)) / open_space_conf.warm_start_config.phi_grid_resolution);
...
Box2d Node3d::GetBoundingBox(const common::VehicleParam &vehicle_param_, const double x,
