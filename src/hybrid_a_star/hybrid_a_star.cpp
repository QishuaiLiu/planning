...
HybridAStar::HybridAStar(const PlannerOpenSpaceConfig &open_space_conf) {
    ...
    arc_length_ = planner_open_space_config_.warm_start_config.phi_grid_resolution *
                  ...
                  std::tan(max_steer_angle_ * 2 / (static_cast<double>(next_node_num_) / 2 - 1));
    ...
    acc_weight_ =
        planner_open_space_config_.iterative_anchoring_smoother_config.s_curve_config.acc_weight;
    jerk_weight_ =
        planner_open_space_config_.iterative_anchoring_smoother_config.s_curve_config.jerk_weight;
    kappa_penalty_weight_ = planner_open_space_config_.iterative_anchoring_smoother_config
                                .s_curve_config.kappa_penalty_weight;
    ref_s_weight_ =
        planner_open_space_config_.iterative_anchoring_smoother_config.s_curve_config.ref_s_weight;
    ref_v_weight_ =
        planner_open_space_config_.iterative_anchoring_smoother_config.s_curve_config.ref_v_weight;
...
bool HybridAStar::ValidityCheck(std::shared_ptr<Node3d> node) {
    ...
    for (size_t i = check_start_index; i < node_step_size; ++i) {
        ...
        Box2d bounding_box = Node3d::GetBoundingBox(vehicle_param_, traversed_x[i], traversed_y[i],
                                                    traversed_phi[i]);
...
std::shared_ptr<Node3d> HybridAStar::LoadRSPinCS(
    const std::shared_ptr<ReedSheppPath> reeds_shepp_to_end, std::shared_ptr<Node3d> current_node) {
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(std::shared_ptr<Node3d> current_node,
    ...
    for (size_t i = 0; i < arc_length_ / step_size_; ++i) {
        ...
        intermediate_phi.push_back(math::NormalizeAngle(next_phi));
        last_x = next_x;
        last_y = next_y;
        last_phi = next_phi;
    }
...
bool HybridAStar::GetResult(HybridAStartResult *result) {
    ...
    // current_node->GetYs().back(); AINFO << "switch node:" << current_node->GetXs().front() << ",
    // "
    ...
    if (result->x.size() != result->y.size() || result->x.size() != result->v.size() ||
        ...
        //        result->y.size()
        //        << "result->phi.size()" << result->phi.size() << "result->v.size()"
        //        << result->v.size();
        return false;
    }
...
//     const double gear =
//         std::abs(math::NormalizeAngle(init_heading - init_tracking_vector.Angle())) < M_PI_2;
...
//         std::max(gear ? 1.5 * (max_forward_v_ * max_forward_v_ + path_length * max_forward_acc_)
//         /
...
//         // AERROR << "Number of knots overflow. total_t: " << total_t << ", delta_t: " <<
//         delta_t_; return false;
//     }
//     const size_t num_of_knots = static_cast<size_t>(total_t / delta_t_) + 1;

//     PiecewiseJerkSpeedProblem piecewise_jerk_problem(num_of_knots, delta_t_,
...
//             // << static_cast<double>(i) * delta_t_ << "with total time " << total_t;
//             break;
//         }
//         // speed_data.AppendSpeedPoint(s[i], delta_t_ * static_cast<double>(i), ds[i], dds[i],
//                                     (dds[i] - dds[i - 1]) / delta_t_);
...
//                                     discrete_steer = gear ? std::atan(discrete_steer)
//                                                           : std::atan(-discrete_steer);
...
bool HybridAStar::TrajectoryPartition(const HybridAStartResult &result,
    ...
    if (x.size() != y.size() || x.size() != phi.size()) {
        // AERROR << "states sizes are not equal when do trajectory
        // partitioning of "
    ...
    bool current_gear = std::abs(math::NormalizeAngle(tracking_angle - heading_angle)) < (M_PI_2);
    for (size_t i = 0; i < horizon - 1; ++i) {
        heading_angle = phi[i];
        const Vec2d tracking_vector(x[i + 1] - x[i], y[i + 1] - y[i]);
        tracking_angle = tracking_vector.Angle();
        bool gear = std::abs(math::NormalizeAngle(tracking_angle - heading_angle)) < (M_PI_2);
...
bool HybridAStar::Plan(double sx, double sy, double sphi, double ex, double ey, double ephi,
    ...
    auto map_time = std::chrono::steady_clock::now();
    grid_a_star_heuristic_generator_->GenerateDpMap(ex, ey, XYbounds_, obstacles_linesegments_vec_);
    auto end_map_time = std::chrono::steady_clock::now();
    std::cout << "map time " << (end_map_time - map_time).count();
    // load open set, pq
    ...
    uint64_t heuristic_time;
    uint64_t node_generator_time;
    uint64_t validity_check_time;
    uint64_t rs_time;

    size_t max_explored_num = planner_open_space_config_.warm_start_config.max_explored_num;
    size_t desired_explored_num =
        std::min(planner_open_space_config_.warm_start_config.desired_explored_num,
    ...
    while (!open_pq_.empty() && open_pq_.size() < kMaxNodeNum &&
           available_result_num < desired_explored_num && explored_node_num < max_explored_num) {
        std::shared_ptr<Node3d> current_node = open_pq_.top().first;
        open_pq_.pop();
        const auto rs_start_time = std::chrono::steady_clock::now();
        ...
        explored_node_num++;
        const auto rs_end_time = std::chrono::steady_clock::now();
        rs_time = std::chrono::duration_cast<std::chrono::milliseconds>(rs_end_time - rs_start_time)
                      .count();
        close_set_.insert(current_node->GetIndex());

        auto end_astar_time = std::chrono::steady_clock::now();

        if (std::chrono::duration_cast<std::chrono::seconds>(end_astar_time - astar_start_time)
                    .count() > planner_open_space_config_.warm_start_config.astar_max_search_time &&
        ...
        for (size_t i = begin_index; i < end_index; ++i) {
            const auto gen_node_time = std::chrono::steady_clock::now();
            std::shared_ptr<Node3d> next_node = Next_node_generator(current_node, i);
            node_generator_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                                      std::chrono::steady_clock::now() - gen_node_time)
                                      .count();
            ...
            // collision check
            const auto validity_check_start_time = std::chrono::steady_clock::now();
            if (!ValidityCheck(next_node)) {
                continue;
            }
            validity_check_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                                      std::chrono::steady_clock::now() - validity_check_start_time)
                                      .count();
    ...
    std::cout << "open_pq_.empty()" << (open_pq_.empty() ? "true" : "false") << "\n";
    std::cout << "open_pq_.size()" << open_pq_.size() << "\n";
    std::cout << "desired_explored_num" << desired_explored_num << "\n";
    std::cout << "min cost is : " << final_node_->GetTrajCost() << "\n";
    std::cout << "max_explored_num is " << max_explored_num << "\n";
    std::cout << "explored node num is " << explored_node_num << "available_result_num "
              << available_result_num << "\n";
    std::cout << "best_explored_num is " << best_explored_num << "best_available_result_num is "
              << best_available_result_num << "\n";
    std::cout << "cal node time is " << heuristic_time << "validity_check_time "
    ...
    if (final_node_ == nullptr) {
        // AERROR << "Hybrid A searching return null ptr(open_set ran
        // out)";
        print_curves.PrintToLog();
        return false;
    }
