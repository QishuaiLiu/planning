...
#include "hybrid_a_star/reeds_shepp_path.h"
...
std::pair<double, double> ReedShepp::calc_tau_omega(const double u, const double v, const double xi,
...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
    ...
    for (size_t i = 0; i < paths_size; ++i) {
        ...
        for (size_t j = 0; j < path.segs_lengths.size(); j++) {
            if (path.segs_types[j] != 'S') {
                cost += std::fabs(path.segs_lengths[j]) * (traj_steer_penalty_) / max_kappa_ *
                        path.radius;
                ...
                if (std::fabs(path.segs_lengths[j]) / max_kappa_ < traj_expected_shortest_length_) {
                    cost += traj_short_length_penalty_;
                }
            }
    ...
    if (std::abs(all_possible_paths[optimal_path_index].x.back() - end_node->GetX()) > 1e-3 ||
        ...
        math::NormalizeAngle(all_possible_paths[optimal_path_index].phi.back() -
...
bool ReedShepp::CS(const double x, const double y, const double phi,
    ...
    char LS1_types[] = "LS";
    if (LS1_param.flag && !SetRSP(2, LS1_lengths, LS1_types, all_possible_paths, LS1_param.v)) {
    ...
    char LS2_types[] = "LS";
    if (LS2_param.flag && !SetRSP(2, LS2_lengths, LS2_types, all_possible_paths, LS2_param.v)) {
        // ADEBUG << "Fail at SetRSP with LS_param";
        return false;
    }
    RSPParam RS1_param;
    ...
    char RS1_types[] = "RS";
    if (RS1_param.flag && !SetRSP(2, RS1_lengths, RS1_types, all_possible_paths, RS1_param.v)) {
        // ADEBUG << "Fail at SetRSP with SLS_param";
        return false;
    }
    ...
    char RS2_types[] = "RS";
    if (RS2_param.flag && !SetRSP(2, RS2_lengths, RS2_types, all_possible_paths, RS2_param.v)) {
        // ADEBUG << "Fail at SetRSP with SLS_param";
        return false;
    }
    return true;
...
void ReedShepp::LS(const double x, const double y, const double phi, RSPParam *param) {
    double phi_mod = math::NormalizeAngle(phi);
    double epsilon = 1e-6;
    if (std::fabs(phi_mod - M_PI_2) < epsilon || std::fabs(phi_mod) < epsilon ||
        std::fabs(phi_mod + M_PI_2) < epsilon || std::fabs(phi_mod + M_PI) < epsilon) {
        return;
    }
...
bool ReedShepp::SetRSP(const int size, const double *lengths, const char *types,
                       std::vector<ReedSheppPath> *all_possible_paths, double radius) {
    ReedSheppPath path;
...
bool ReedShepp::GenerateLocalConfigurations(const std::shared_ptr<Node3d> start_node,
    ...
    for (size_t i = 0; i < shortest_path->segs_types.size(); ++i) {
        ...
        index--;
        if (i >= 1 &&
            shortest_path->segs_lengths.at(i - 1) * shortest_path->segs_lengths.at(i) > 0) {
            pd = -d - ll;
        } else {
            pd = d - ll;
        }
        while (std::abs(pd) <= std::abs(l)) {
            index++;
            Interpolation(index, pd, m, ox, oy, ophi, radius, &px, &py, &pphi, &pgear);
            pd += d;
        }
        ll = l - pd - d;
        index++;
    ...
    for (size_t i = 0; i < px.size(); ++i) {
        ...
        shortest_path->phi.push_back(math::NormalizeAngle(pphi.at(i) + start_node->GetPhi()));
    }
...
void ReedShepp::Interpolation(const int index, const double pd, const char m, const double ox,
...
bool ReedShepp::SetRSPPar(const int size, const double *lengths, const std::string &types,
